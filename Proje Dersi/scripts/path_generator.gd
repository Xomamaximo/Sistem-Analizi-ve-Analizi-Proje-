...
var zorluk_tablosu = {"kolay":50, "orta":36 , "zor":20  } 
...
func _init():
	generate_path()
	while _path_route.size() <  zorluk_tablosu[path_config.zorluk] or  _path_route.size() >  zorluk_tablosu[path_config.zorluk] +4  :
		generate_path()
